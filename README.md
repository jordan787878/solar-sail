...
<table>
  <tr>
   <td>Purple: only motion planner. Red: motion planner + LQR update <img src="https://github.com/jordan787878/solar-sail/assets/17584164/d578e824-b3c6-45b1-afa1-5dcd53e10f0d" alt="Image 1" style="width: 500px; height: 400px;"></td>
...
## Abstraction Control Synthesis
### MDP and Value Iteration
We construct a MDP=(Q,A,P,R) as the discrete abstraction of the dynamical systems. The discrete (and finite) state in MDP is constructed from the reference trajectory generated by the motion planner.
The transition function P is constructed by forward simulating from a pair of (q, a) over a fixed time duration. 
For each (q,a) pair, the conitnuous states that are the "single" positive/negative and "complete" positive/negative are used.
P is implemented as a sparse matrix, where the row is the (q,a) pair, the column is the (q new), and the element is the transition probability.
The reward function R is constructed by: if q is in goal region, reward(q) = 1.0. If q is the fail region, reward(q) = -1.0. Else, reward(q) = 0.0.
After the MDP construction, we conduct value iteration until convergence to epilson.
Finally, the optimal feedback control policy is constructed by solving the Bellman equation.

Below we show the result of motion plannint + MDP control synthesis of run-time control. Note that we use the AO planner to ensure the quality of the reference trajectory.
Left plot shows the reference trajectory (blue) and the run-time MDP trajectory (red). The discrete state partition is shown as gray patches.
Middle plot visualize the sparse matrix of P. Right plot shows the MDP control trajectory.
<table>
  <tr>
   <td><img src="images/marine_vessel_mdp_traj.png" alt="Image 1" style="width: 400px; height: 300px;"></td>
    <td><img src="images/marine_vessel_mdp_transition.png" alt="Image 2" style="width: 400px; height: 300px;"></td>
    <td><img src="images/marine_vessel_mdp_control.png" alt="Image 1" style="width: 400px; height: 300px;"></td>
  </tr>
</table>

The next step is to analyze formal guarantee. Because sometimes it fails: the synthesized control policy may still drive the state to a point that is not included in the MDP finite state.
Another step is to incorporate process noise while constructing the P mapping.
